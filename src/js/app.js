...
function iniciarApp() {
    ...
    seleccinarFecha(); // Añade la fecha de la cita en el objeto
    seleccionarHora(); // Añade la hora de la cita en el objeto

    mostrarResumen(); // Muestra los datos de la cita
}
...
function  seleccionarHora() { 
    ...
    inputHora.addEventListener("input", function(e){
        ...
        if(hora < 10 || hora > 18) {
            e.target.value = "";
            mostrarAlerta("Hora Inválida", "error", ".formulario");
        } else {
...
function mostrarResumen() {
    ...
    const fechFormateada = fechaUTC.toLocaleDateString("es-AR", opciones);

    const fechaCita = document.createElement("P");
    fechaCita.innerHTML = `<span>Fecha:</span> ${fechFormateada}`;

    const fechaHora = document.createElement("P");
    fechaHora.innerHTML = `<span>Hora:</span> ${hora}`;

    const precioTotal = servicios.reduce((precioTotal, servicio) => precioTotal + 
        parseFloat(servicio.precio), 0 );
        const textoTotal = document.createElement("P");
        textoTotal.innerHTML = `<span>Total:</span> $${precioTotal}`;
